...
vibe-cli-kit 不追求做成一个庞杂的个人配置框架，而是提供一个轻量、可用、开箱即用的终端基线。它会一次性安装并配置 `ghostty` `yazi` `tmux` `zellij` `fzf` `fd` `bat` `ripgrep` `lazygit` `nvim` `atuin` `zoxide` `starship` `glow` `fastfetch` `btop`，并写入 shell 快捷命令与本地帮助文档。
...
- 保持轻量，不把项目做成配置垃圾场
- 围绕终端编码工作流，而不是单纯堆工具
- 默认配好 `ghostty`、`tmux`、`zellij`、`yazi`
- 附带 `v` `e` `fif` `p` `tmx` `tmn` `tma` `tml` `zmx` `zmn` `zma` `zml` `y` 等常用命令
- 同时支持 `brew` 和 `apt`
- 内置本地模板库，支持 `v doctor`、`v backup`、`v diff`、`v sync`、`v update`、`v project`、`v r`、`v session`
- 会按系统适配，不可用工具自动跳过并汇总
- 安装结束会汇总：已安装 / 已跳过 / 不可用
...
zmn                          # 当前目录开/进 zellij session
y                            # 打开 Yazi
tmx dev                      # 进入 dev session
zmx dev                      # 进入 dev zellij session
rg foo                       # 全局搜索文本
rg --files | fzf             # 模糊选文件
z foo                        # 快速跳目录
lazygit                      # 打开 Git TUI
...
- `zmx`：智能进入 session
- `zmn`：以当前目录名创建或进入 session
- `zma`：attach 或新建指定 session
- `zml`：列出 session
...
- 检查模板库是否存在
- 检查 `ghostty` / `starship` / `tmux` / `yazi` 配置是否落地
- 检查 `terminal-cheatsheet`、`tmx`、`zmx`、`v`、`e`、`fif` 是否已安装
- 检查 `~/.zshrc` 受管区块是否存在
- 检查常用工具命令是否可用
